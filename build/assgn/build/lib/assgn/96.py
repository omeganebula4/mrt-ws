...
class ObstacleAvoidanceNode(Node):
    ...
    def lidar_callback(self, scan_data):
        ...
        # Right side (225° to 270°)
        right_side = min(scan_data.ranges[315:360])
        ...
        if front < self.safe_distance and left_side < self.safe_distance and right_side < self.safe_distance:
            ...
            self.twist.linear.x = -0.5  # Fill blank 1: Linear velocity for backward motion
            ...
            if left_side > right_side:
                ...
                self.twist.angular.z = 1.0  # Fill blank 2: Angular velocity for left turn
            else:
                self.get_logger().info("Obstacle in front. Turning right.")
                self.twist.angular.z = -1.0  # Fill blank 3: Angular velocity for right turn
            self.twist.linear.x = 0.0  # Stop moving forward
        else:
            # Path is clear, move forward
            self.twist.linear.x = 0.5  # Fill blank 4: Linear velocity for forward motion
            self.twist.angular.z = 0.0
...
def main(args=None):
    ...
    # Fill blank 5: Instance of the ObstacleAvoidanceNode class
