...
class Metaclass_Waypoint(type):
    ...
    def __import_type_support__(cls):
        try:
            ...
            module = import_type_support('all_interfaces')
        except ImportError:
            import logging
            import traceback
            logger = logging.getLogger(
                'all_interfaces.msg.Waypoint')
            logger.debug(
...
class Waypoint(metaclass=Metaclass_Waypoint):
    ...
    def __init__(self, **kwargs):
        ...
        self.t = array.array('d', kwargs.get('t', []))
    ...
    def t(self, value):
        if isinstance(value, array.array):
            assert value.typecode == 'd', \
                "The 't' array.array() must have the type code of 'd'"
            self._t = value
            return
        if __debug__:
            from collections.abc import Sequence
            ...
            assert \
                ((isinstance(value, Sequence) or
                 ...
                 all(isinstance(v, float) for v in value) and
                 all(not (val < -1.7976931348623157e+308 or val > 1.7976931348623157e+308) or math.isinf(val) for val in value)), \
                "The 't' field must be a set or sequence and each value of type 'float' and each double in [-.000000, .000000]"
        self._t = array.array('d', value)
