// generated from rosidl_generator_c/resource/idl__functions.c.em
// with input from pos_control_interfaces:action/Trajectory.idl
// generated code does not contain a copyright notice
#include "pos_control_interfaces/action/detail/trajectory__functions.h"

#include <assert.h>
#include <stdbool.h>
#include <stdlib.h>
#include <string.h>

#include "rcutils/allocator.h"


// Include directives for member types
// Member `command`
#include "std_msgs/msg/detail/string__functions.h"

bool
pos_control_interfaces__action__Trajectory_Goal__init(pos_control_interfaces__action__Trajectory_Goal * msg)
{
  if (!msg) {
    return false;
  }
  // command
  if (!std_msgs__msg__String__init(&msg->command)) {
    pos_control_interfaces__action__Trajectory_Goal__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_Goal__fini(pos_control_interfaces__action__Trajectory_Goal * msg)
{
  if (!msg) {
    return;
  }
  // command
  std_msgs__msg__String__fini(&msg->command);
}

bool
pos_control_interfaces__action__Trajectory_Goal__are_equal(const pos_control_interfaces__action__Trajectory_Goal * lhs, const pos_control_interfaces__action__Trajectory_Goal * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // command
  if (!std_msgs__msg__String__are_equal(
      &(lhs->command), &(rhs->command)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_Goal__copy(
  const pos_control_interfaces__action__Trajectory_Goal * input,
  pos_control_interfaces__action__Trajectory_Goal * output)
{
  if (!input || !output) {
    return false;
  }
  // command
  if (!std_msgs__msg__String__copy(
      &(input->command), &(output->command)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_Goal *
pos_control_interfaces__action__Trajectory_Goal__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Goal * msg = (pos_control_interfaces__action__Trajectory_Goal *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_Goal), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_Goal));
  bool success = pos_control_interfaces__action__Trajectory_Goal__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_Goal__destroy(pos_control_interfaces__action__Trajectory_Goal * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_Goal__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_Goal__Sequence__init(pos_control_interfaces__action__Trajectory_Goal__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Goal * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_Goal *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_Goal), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_Goal__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_Goal__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_Goal__Sequence__fini(pos_control_interfaces__action__Trajectory_Goal__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_Goal__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_Goal__Sequence *
pos_control_interfaces__action__Trajectory_Goal__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Goal__Sequence * array = (pos_control_interfaces__action__Trajectory_Goal__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_Goal__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_Goal__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_Goal__Sequence__destroy(pos_control_interfaces__action__Trajectory_Goal__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_Goal__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_Goal__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_Goal__Sequence * lhs, const pos_control_interfaces__action__Trajectory_Goal__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_Goal__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_Goal__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_Goal__Sequence * input,
  pos_control_interfaces__action__Trajectory_Goal__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_Goal);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_Goal * data =
      (pos_control_interfaces__action__Trajectory_Goal *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_Goal__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_Goal__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_Goal__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


bool
pos_control_interfaces__action__Trajectory_Result__init(pos_control_interfaces__action__Trajectory_Result * msg)
{
  if (!msg) {
    return false;
  }
  // success
  return true;
}

void
pos_control_interfaces__action__Trajectory_Result__fini(pos_control_interfaces__action__Trajectory_Result * msg)
{
  if (!msg) {
    return;
  }
  // success
}

bool
pos_control_interfaces__action__Trajectory_Result__are_equal(const pos_control_interfaces__action__Trajectory_Result * lhs, const pos_control_interfaces__action__Trajectory_Result * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // success
  if (lhs->success != rhs->success) {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_Result__copy(
  const pos_control_interfaces__action__Trajectory_Result * input,
  pos_control_interfaces__action__Trajectory_Result * output)
{
  if (!input || !output) {
    return false;
  }
  // success
  output->success = input->success;
  return true;
}

pos_control_interfaces__action__Trajectory_Result *
pos_control_interfaces__action__Trajectory_Result__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Result * msg = (pos_control_interfaces__action__Trajectory_Result *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_Result), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_Result));
  bool success = pos_control_interfaces__action__Trajectory_Result__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_Result__destroy(pos_control_interfaces__action__Trajectory_Result * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_Result__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_Result__Sequence__init(pos_control_interfaces__action__Trajectory_Result__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Result * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_Result *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_Result), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_Result__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_Result__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_Result__Sequence__fini(pos_control_interfaces__action__Trajectory_Result__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_Result__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_Result__Sequence *
pos_control_interfaces__action__Trajectory_Result__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Result__Sequence * array = (pos_control_interfaces__action__Trajectory_Result__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_Result__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_Result__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_Result__Sequence__destroy(pos_control_interfaces__action__Trajectory_Result__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_Result__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_Result__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_Result__Sequence * lhs, const pos_control_interfaces__action__Trajectory_Result__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_Result__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_Result__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_Result__Sequence * input,
  pos_control_interfaces__action__Trajectory_Result__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_Result);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_Result * data =
      (pos_control_interfaces__action__Trajectory_Result *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_Result__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_Result__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_Result__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `end_effector_pos`
#include "geometry_msgs/msg/detail/point__functions.h"

bool
pos_control_interfaces__action__Trajectory_Feedback__init(pos_control_interfaces__action__Trajectory_Feedback * msg)
{
  if (!msg) {
    return false;
  }
  // end_effector_pos
  if (!geometry_msgs__msg__Point__init(&msg->end_effector_pos)) {
    pos_control_interfaces__action__Trajectory_Feedback__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_Feedback__fini(pos_control_interfaces__action__Trajectory_Feedback * msg)
{
  if (!msg) {
    return;
  }
  // end_effector_pos
  geometry_msgs__msg__Point__fini(&msg->end_effector_pos);
}

bool
pos_control_interfaces__action__Trajectory_Feedback__are_equal(const pos_control_interfaces__action__Trajectory_Feedback * lhs, const pos_control_interfaces__action__Trajectory_Feedback * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // end_effector_pos
  if (!geometry_msgs__msg__Point__are_equal(
      &(lhs->end_effector_pos), &(rhs->end_effector_pos)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_Feedback__copy(
  const pos_control_interfaces__action__Trajectory_Feedback * input,
  pos_control_interfaces__action__Trajectory_Feedback * output)
{
  if (!input || !output) {
    return false;
  }
  // end_effector_pos
  if (!geometry_msgs__msg__Point__copy(
      &(input->end_effector_pos), &(output->end_effector_pos)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_Feedback *
pos_control_interfaces__action__Trajectory_Feedback__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Feedback * msg = (pos_control_interfaces__action__Trajectory_Feedback *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_Feedback), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_Feedback));
  bool success = pos_control_interfaces__action__Trajectory_Feedback__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_Feedback__destroy(pos_control_interfaces__action__Trajectory_Feedback * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_Feedback__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_Feedback__Sequence__init(pos_control_interfaces__action__Trajectory_Feedback__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Feedback * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_Feedback *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_Feedback), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_Feedback__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_Feedback__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_Feedback__Sequence__fini(pos_control_interfaces__action__Trajectory_Feedback__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_Feedback__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_Feedback__Sequence *
pos_control_interfaces__action__Trajectory_Feedback__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_Feedback__Sequence * array = (pos_control_interfaces__action__Trajectory_Feedback__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_Feedback__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_Feedback__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_Feedback__Sequence__destroy(pos_control_interfaces__action__Trajectory_Feedback__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_Feedback__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_Feedback__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_Feedback__Sequence * lhs, const pos_control_interfaces__action__Trajectory_Feedback__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_Feedback__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_Feedback__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_Feedback__Sequence * input,
  pos_control_interfaces__action__Trajectory_Feedback__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_Feedback);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_Feedback * data =
      (pos_control_interfaces__action__Trajectory_Feedback *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_Feedback__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_Feedback__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_Feedback__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
#include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `goal`
// already included above
// #include "pos_control_interfaces/action/detail/trajectory__functions.h"

bool
pos_control_interfaces__action__Trajectory_SendGoal_Request__init(pos_control_interfaces__action__Trajectory_SendGoal_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(msg);
    return false;
  }
  // goal
  if (!pos_control_interfaces__action__Trajectory_Goal__init(&msg->goal)) {
    pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(pos_control_interfaces__action__Trajectory_SendGoal_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // goal
  pos_control_interfaces__action__Trajectory_Goal__fini(&msg->goal);
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Request__are_equal(const pos_control_interfaces__action__Trajectory_SendGoal_Request * lhs, const pos_control_interfaces__action__Trajectory_SendGoal_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // goal
  if (!pos_control_interfaces__action__Trajectory_Goal__are_equal(
      &(lhs->goal), &(rhs->goal)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Request__copy(
  const pos_control_interfaces__action__Trajectory_SendGoal_Request * input,
  pos_control_interfaces__action__Trajectory_SendGoal_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // goal
  if (!pos_control_interfaces__action__Trajectory_Goal__copy(
      &(input->goal), &(output->goal)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_SendGoal_Request *
pos_control_interfaces__action__Trajectory_SendGoal_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_SendGoal_Request * msg = (pos_control_interfaces__action__Trajectory_SendGoal_Request *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Request));
  bool success = pos_control_interfaces__action__Trajectory_SendGoal_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Request__destroy(pos_control_interfaces__action__Trajectory_SendGoal_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__init(pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_SendGoal_Request * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_SendGoal_Request *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_SendGoal_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__fini(pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence *
pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * array = (pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__destroy(pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * lhs, const pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_SendGoal_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * input,
  pos_control_interfaces__action__Trajectory_SendGoal_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_SendGoal_Request * data =
      (pos_control_interfaces__action__Trajectory_SendGoal_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_SendGoal_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_SendGoal_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_SendGoal_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `stamp`
#include "builtin_interfaces/msg/detail/time__functions.h"

bool
pos_control_interfaces__action__Trajectory_SendGoal_Response__init(pos_control_interfaces__action__Trajectory_SendGoal_Response * msg)
{
  if (!msg) {
    return false;
  }
  // accepted
  // stamp
  if (!builtin_interfaces__msg__Time__init(&msg->stamp)) {
    pos_control_interfaces__action__Trajectory_SendGoal_Response__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Response__fini(pos_control_interfaces__action__Trajectory_SendGoal_Response * msg)
{
  if (!msg) {
    return;
  }
  // accepted
  // stamp
  builtin_interfaces__msg__Time__fini(&msg->stamp);
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Response__are_equal(const pos_control_interfaces__action__Trajectory_SendGoal_Response * lhs, const pos_control_interfaces__action__Trajectory_SendGoal_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // accepted
  if (lhs->accepted != rhs->accepted) {
    return false;
  }
  // stamp
  if (!builtin_interfaces__msg__Time__are_equal(
      &(lhs->stamp), &(rhs->stamp)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Response__copy(
  const pos_control_interfaces__action__Trajectory_SendGoal_Response * input,
  pos_control_interfaces__action__Trajectory_SendGoal_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // accepted
  output->accepted = input->accepted;
  // stamp
  if (!builtin_interfaces__msg__Time__copy(
      &(input->stamp), &(output->stamp)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_SendGoal_Response *
pos_control_interfaces__action__Trajectory_SendGoal_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_SendGoal_Response * msg = (pos_control_interfaces__action__Trajectory_SendGoal_Response *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Response));
  bool success = pos_control_interfaces__action__Trajectory_SendGoal_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Response__destroy(pos_control_interfaces__action__Trajectory_SendGoal_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_SendGoal_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__init(pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_SendGoal_Response * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_SendGoal_Response *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_SendGoal_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_SendGoal_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__fini(pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_SendGoal_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence *
pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * array = (pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__destroy(pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * lhs, const pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_SendGoal_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * input,
  pos_control_interfaces__action__Trajectory_SendGoal_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_SendGoal_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_SendGoal_Response * data =
      (pos_control_interfaces__action__Trajectory_SendGoal_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_SendGoal_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_SendGoal_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_SendGoal_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"

bool
pos_control_interfaces__action__Trajectory_GetResult_Request__init(pos_control_interfaces__action__Trajectory_GetResult_Request * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    pos_control_interfaces__action__Trajectory_GetResult_Request__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Request__fini(pos_control_interfaces__action__Trajectory_GetResult_Request * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Request__are_equal(const pos_control_interfaces__action__Trajectory_GetResult_Request * lhs, const pos_control_interfaces__action__Trajectory_GetResult_Request * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Request__copy(
  const pos_control_interfaces__action__Trajectory_GetResult_Request * input,
  pos_control_interfaces__action__Trajectory_GetResult_Request * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_GetResult_Request *
pos_control_interfaces__action__Trajectory_GetResult_Request__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_GetResult_Request * msg = (pos_control_interfaces__action__Trajectory_GetResult_Request *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_GetResult_Request), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_GetResult_Request));
  bool success = pos_control_interfaces__action__Trajectory_GetResult_Request__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Request__destroy(pos_control_interfaces__action__Trajectory_GetResult_Request * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_GetResult_Request__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__init(pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_GetResult_Request * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_GetResult_Request *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_GetResult_Request), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_GetResult_Request__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_GetResult_Request__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__fini(pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_GetResult_Request__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence *
pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * array = (pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__destroy(pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * lhs, const pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_GetResult_Request__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * input,
  pos_control_interfaces__action__Trajectory_GetResult_Request__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_GetResult_Request);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_GetResult_Request * data =
      (pos_control_interfaces__action__Trajectory_GetResult_Request *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_GetResult_Request__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_GetResult_Request__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_GetResult_Request__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `result`
// already included above
// #include "pos_control_interfaces/action/detail/trajectory__functions.h"

bool
pos_control_interfaces__action__Trajectory_GetResult_Response__init(pos_control_interfaces__action__Trajectory_GetResult_Response * msg)
{
  if (!msg) {
    return false;
  }
  // status
  // result
  if (!pos_control_interfaces__action__Trajectory_Result__init(&msg->result)) {
    pos_control_interfaces__action__Trajectory_GetResult_Response__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Response__fini(pos_control_interfaces__action__Trajectory_GetResult_Response * msg)
{
  if (!msg) {
    return;
  }
  // status
  // result
  pos_control_interfaces__action__Trajectory_Result__fini(&msg->result);
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Response__are_equal(const pos_control_interfaces__action__Trajectory_GetResult_Response * lhs, const pos_control_interfaces__action__Trajectory_GetResult_Response * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // status
  if (lhs->status != rhs->status) {
    return false;
  }
  // result
  if (!pos_control_interfaces__action__Trajectory_Result__are_equal(
      &(lhs->result), &(rhs->result)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Response__copy(
  const pos_control_interfaces__action__Trajectory_GetResult_Response * input,
  pos_control_interfaces__action__Trajectory_GetResult_Response * output)
{
  if (!input || !output) {
    return false;
  }
  // status
  output->status = input->status;
  // result
  if (!pos_control_interfaces__action__Trajectory_Result__copy(
      &(input->result), &(output->result)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_GetResult_Response *
pos_control_interfaces__action__Trajectory_GetResult_Response__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_GetResult_Response * msg = (pos_control_interfaces__action__Trajectory_GetResult_Response *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_GetResult_Response), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_GetResult_Response));
  bool success = pos_control_interfaces__action__Trajectory_GetResult_Response__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Response__destroy(pos_control_interfaces__action__Trajectory_GetResult_Response * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_GetResult_Response__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__init(pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_GetResult_Response * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_GetResult_Response *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_GetResult_Response), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_GetResult_Response__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_GetResult_Response__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__fini(pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_GetResult_Response__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence *
pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * array = (pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__destroy(pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * lhs, const pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_GetResult_Response__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * input,
  pos_control_interfaces__action__Trajectory_GetResult_Response__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_GetResult_Response);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_GetResult_Response * data =
      (pos_control_interfaces__action__Trajectory_GetResult_Response *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_GetResult_Response__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_GetResult_Response__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_GetResult_Response__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}


// Include directives for member types
// Member `goal_id`
// already included above
// #include "unique_identifier_msgs/msg/detail/uuid__functions.h"
// Member `feedback`
// already included above
// #include "pos_control_interfaces/action/detail/trajectory__functions.h"

bool
pos_control_interfaces__action__Trajectory_FeedbackMessage__init(pos_control_interfaces__action__Trajectory_FeedbackMessage * msg)
{
  if (!msg) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__init(&msg->goal_id)) {
    pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(msg);
    return false;
  }
  // feedback
  if (!pos_control_interfaces__action__Trajectory_Feedback__init(&msg->feedback)) {
    pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(msg);
    return false;
  }
  return true;
}

void
pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(pos_control_interfaces__action__Trajectory_FeedbackMessage * msg)
{
  if (!msg) {
    return;
  }
  // goal_id
  unique_identifier_msgs__msg__UUID__fini(&msg->goal_id);
  // feedback
  pos_control_interfaces__action__Trajectory_Feedback__fini(&msg->feedback);
}

bool
pos_control_interfaces__action__Trajectory_FeedbackMessage__are_equal(const pos_control_interfaces__action__Trajectory_FeedbackMessage * lhs, const pos_control_interfaces__action__Trajectory_FeedbackMessage * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__are_equal(
      &(lhs->goal_id), &(rhs->goal_id)))
  {
    return false;
  }
  // feedback
  if (!pos_control_interfaces__action__Trajectory_Feedback__are_equal(
      &(lhs->feedback), &(rhs->feedback)))
  {
    return false;
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_FeedbackMessage__copy(
  const pos_control_interfaces__action__Trajectory_FeedbackMessage * input,
  pos_control_interfaces__action__Trajectory_FeedbackMessage * output)
{
  if (!input || !output) {
    return false;
  }
  // goal_id
  if (!unique_identifier_msgs__msg__UUID__copy(
      &(input->goal_id), &(output->goal_id)))
  {
    return false;
  }
  // feedback
  if (!pos_control_interfaces__action__Trajectory_Feedback__copy(
      &(input->feedback), &(output->feedback)))
  {
    return false;
  }
  return true;
}

pos_control_interfaces__action__Trajectory_FeedbackMessage *
pos_control_interfaces__action__Trajectory_FeedbackMessage__create()
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_FeedbackMessage * msg = (pos_control_interfaces__action__Trajectory_FeedbackMessage *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_FeedbackMessage), allocator.state);
  if (!msg) {
    return NULL;
  }
  memset(msg, 0, sizeof(pos_control_interfaces__action__Trajectory_FeedbackMessage));
  bool success = pos_control_interfaces__action__Trajectory_FeedbackMessage__init(msg);
  if (!success) {
    allocator.deallocate(msg, allocator.state);
    return NULL;
  }
  return msg;
}

void
pos_control_interfaces__action__Trajectory_FeedbackMessage__destroy(pos_control_interfaces__action__Trajectory_FeedbackMessage * msg)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (msg) {
    pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(msg);
  }
  allocator.deallocate(msg, allocator.state);
}


bool
pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__init(pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * array, size_t size)
{
  if (!array) {
    return false;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_FeedbackMessage * data = NULL;

  if (size) {
    data = (pos_control_interfaces__action__Trajectory_FeedbackMessage *)allocator.zero_allocate(size, sizeof(pos_control_interfaces__action__Trajectory_FeedbackMessage), allocator.state);
    if (!data) {
      return false;
    }
    // initialize all array elements
    size_t i;
    for (i = 0; i < size; ++i) {
      bool success = pos_control_interfaces__action__Trajectory_FeedbackMessage__init(&data[i]);
      if (!success) {
        break;
      }
    }
    if (i < size) {
      // if initialization failed finalize the already initialized array elements
      for (; i > 0; --i) {
        pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(&data[i - 1]);
      }
      allocator.deallocate(data, allocator.state);
      return false;
    }
  }
  array->data = data;
  array->size = size;
  array->capacity = size;
  return true;
}

void
pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__fini(pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * array)
{
  if (!array) {
    return;
  }
  rcutils_allocator_t allocator = rcutils_get_default_allocator();

  if (array->data) {
    // ensure that data and capacity values are consistent
    assert(array->capacity > 0);
    // finalize all array elements
    for (size_t i = 0; i < array->capacity; ++i) {
      pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(&array->data[i]);
    }
    allocator.deallocate(array->data, allocator.state);
    array->data = NULL;
    array->size = 0;
    array->capacity = 0;
  } else {
    // ensure that data, size, and capacity values are consistent
    assert(0 == array->size);
    assert(0 == array->capacity);
  }
}

pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence *
pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__create(size_t size)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * array = (pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence *)allocator.allocate(sizeof(pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence), allocator.state);
  if (!array) {
    return NULL;
  }
  bool success = pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__init(array, size);
  if (!success) {
    allocator.deallocate(array, allocator.state);
    return NULL;
  }
  return array;
}

void
pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__destroy(pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * array)
{
  rcutils_allocator_t allocator = rcutils_get_default_allocator();
  if (array) {
    pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__fini(array);
  }
  allocator.deallocate(array, allocator.state);
}

bool
pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__are_equal(const pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * lhs, const pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * rhs)
{
  if (!lhs || !rhs) {
    return false;
  }
  if (lhs->size != rhs->size) {
    return false;
  }
  for (size_t i = 0; i < lhs->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_FeedbackMessage__are_equal(&(lhs->data[i]), &(rhs->data[i]))) {
      return false;
    }
  }
  return true;
}

bool
pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence__copy(
  const pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * input,
  pos_control_interfaces__action__Trajectory_FeedbackMessage__Sequence * output)
{
  if (!input || !output) {
    return false;
  }
  if (output->capacity < input->size) {
    const size_t allocation_size =
      input->size * sizeof(pos_control_interfaces__action__Trajectory_FeedbackMessage);
    rcutils_allocator_t allocator = rcutils_get_default_allocator();
    pos_control_interfaces__action__Trajectory_FeedbackMessage * data =
      (pos_control_interfaces__action__Trajectory_FeedbackMessage *)allocator.reallocate(
      output->data, allocation_size, allocator.state);
    if (!data) {
      return false;
    }
    // If reallocation succeeded, memory may or may not have been moved
    // to fulfill the allocation request, invalidating output->data.
    output->data = data;
    for (size_t i = output->capacity; i < input->size; ++i) {
      if (!pos_control_interfaces__action__Trajectory_FeedbackMessage__init(&output->data[i])) {
        // If initialization of any new item fails, roll back
        // all previously initialized items. Existing items
        // in output are to be left unmodified.
        for (; i-- > output->capacity; ) {
          pos_control_interfaces__action__Trajectory_FeedbackMessage__fini(&output->data[i]);
        }
        return false;
      }
    }
    output->capacity = input->size;
  }
  output->size = input->size;
  for (size_t i = 0; i < input->size; ++i) {
    if (!pos_control_interfaces__action__Trajectory_FeedbackMessage__copy(
        &(input->data[i]), &(output->data[i])))
    {
      return false;
    }
  }
  return true;
}
