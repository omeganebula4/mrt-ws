...
def min_jerk(pos=None, dur=None, vel=None, acc=None, psg=None):
    ...
    if psg is None:                     # passage times unknown, optimize
        if N > 2:
            psg = np.arange(dur/(N-1), dur-dur/(N-1)+1, dur/(N-1))
...
def mjCOST(t, x, v0, a0, t0):
    ...
    # Reshape t to be column vector if needed
    if len(t) > 0:
    ...
    vv0 = vv[0:N-1,:]
    vv1 = vv[1:N,:]
    tt = np.concatenate((t0[0:1], t, t0[1:2]), axis=0)  # Fixed: use slicing instead of indexing
    T = np.diff(tt, axis=0) * np.ones((1,D))
    xx0 = x[0:N-1,:]
    xx1 = x[1:N,:]
...
def mjTRJ(tx, x, v0, a0, t0, P):
    ...
    # Reshape tx to be column vector if needed
    if len(tx) > 0:
    ...
    if len(tx) > 0:
        ...
        aa = np.concatenate(([a0[0,:]], a, [a0[1,:]]), axis=0)
        vv = np.concatenate(([v0[0,:]], v, [v0[1,:]]), axis=0)
        tt = np.concatenate((t0[0:1], tx, t0[1:2]), axis=0)  # Fixed: use slicing
    else:
        aa = a0
        vv = v0
        tt = t0
    ...
    for i in range(1, int(P)+1):
        t = (i-1)/(P-1)*(t0[1,0]-t0[0,0]) + t0[0,0]  # Fixed: access scalar values
        if t > tt[ii+1,0]:  # Fixed: access scalar value
            ii = ii+1
        T = (tt[ii+1,0]-tt[ii,0])*np.ones((1,D))  # Fixed: access scalar values
        t = (t-tt[ii,0])*np.ones((1,D))  # Fixed: access scalar value
        aa0 = aa[ii,:]
        aa1 = aa[ii+1,:]
        vv0 = vv[ii,:]
        vv1 = vv[ii+1,:]
...
def mjVelAcc(t, x, v0, a0, t0):
    ...
    tt = np.concatenate((t0[0:1], t, t0[1:2]), axis=0)  # Fixed: use slicing

    for i in range(1, 2*N-4+1, 2):

        ii = int(math.ceil(i/2.0))
        T0 = tt[ii,0] - tt[ii-1,0]  # Fixed: access scalar values
        T1 = tt[ii+1,0] - tt[ii,0]  # Fixed: access scalar values
    ...
    T0 = tt[1,0] - tt[0,0]  # Fixed: access scalar values
    T1 = tt[N-1,0] - tt[N-2,0]  # Fixed: access scalar values
